...
runcmd(struct cmd *cmd)
{
  ...
  case '|':
    ...
    if(fork1()==0){
      close(1);//Close the std output,
      dup(p[1]);//So when dup is called, it has fd=1
      close(p[0]);
      close(p[1]);
      runcmd(pcmd->left);
    }
    ...
    close(p[0]);
    close(p[1]);
    wait(&r);//why two wait? Because it has two childs
    wait(&r);
    break;
  }    
  _exit(0);
...
redircmd(struct cmd *subcmd, char *file, int type)
{
  ...
  cmd->fd = (type == '<') ? 0 : 1;//redirect to std input. And if fd=1, std output
  return (struct cmd*)cmd;
}
